...
앱 링크만 있는 페이지는 내가 몇 달 뒤에 다시 봐도 당시 결정을 떠올리기 어렵다. 그래서 zozindev.com의 프로젝트 상세 페이지에는 최소한의 기록 기준을 두기로 했다.
...
다만 이미지만 넣고 본문을 줄이면 안 된다. 이미지는 화면을 보여 줄 뿐, 어떤 문제를 해결하는지나 결과를 어떻게 확인해야 하는지는 설명하지 못한다. 실제 캡처와 직접 확인한 수치를 본문에서 같이 다루는 편이 낫다.
