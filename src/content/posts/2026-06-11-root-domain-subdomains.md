...
루트가 특정 앱으로 바로 이동하는 주소가 아니어서 프로젝트가 끝나거나 도메인이 바뀌어도 설명과 기록은 남는다. 방문자는 앱을 열기 전에 목적과 현재 상태를 확인할 수 있다.
...
그리고 루트에는 프로젝트 상세 페이지와 관련 제작 기록을 둔다. 앱 주소만 던져 두면 왜 만들었는지, 데이터는 어디에 저장되는지, 현재 어떤 제한이 있는지 알기 어렵다. 실제로 사용하기 전에 필요한 맥락은 루트에서 설명한다.
