...
src/content/posts/
  ...
  2026-06-11-wtheat-random-menu-project.md
...
기존 글 URL은 이미 sitemap에 들어갔고, 검색 엔진이나 공유 링크에 잡힐 수 있다. 그래서 새 구조로 바꾸면서도 기존 URL 형태는 유지했다.

```text
/blog/2026-06-11-starting-static-blog.html
/projects/wtheat.html
/privacy.html
```

목록 페이지는 Astro 빌드 설정 때문에 `blog.html`, `projects.html`로 생성되지만, Cloudflare Pages의 `_redirects`에서 `/blog/`, `/projects/`로 접근해도 같은 페이지가 보이게 처리했다.
